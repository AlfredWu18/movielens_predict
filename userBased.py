...
def predictRating(recommend_data, data_test, M, sim_user):
    # f = open("toBeRated.csv","r")
    toBeRated = {"user": [], "item": [], "realRating": []}
    for r in data_test:
        toBeRated["item"].append(r[1])
        toBeRated["user"].append(r[0])
        toBeRated['realRating'].append(r[2])
    ...
    for e in range(l):
        ...
        realRating = toBeRated["realRating"][e]

        pred = 3.0

        # user-based
        if user < len(M)  and item < len(M[0]) and np.count_nonzero(M[user - 1]):
            sim = sim_user[user - 1]
            ind = (M[:, item - 1] > 0)
            # ind[user-1] = False
        ...
        # print(str(user) + "," + str(item) + "," + str(realRating) + "," + str(pred))
        # fw.write(str(user) + "," + str(item) + "," + str(pred) + "\n")
        fw_w.write(str(user) + "," + str(item) + "," + str(realRating) + "," + str(pred) + "\n")

        # fw.close()
    fw_w.close()

data_train = readingFile("rating_train.csv")
getMatrix(data_train)
M, sim_user = crossValidation(data_train)
data_test = readingFile("rating_test.csv")
predictRating(data_train, data_test, M, sim_user)


def readingResult(filename):
    f = open(filename, "r")
    data = []
    for row in f:
        r = row.split(',')
        e = [int(r[0]), int(r[1]), float(r[2]), float(r[3])]
        data.append(e)
    return data

result = readingResult("result1.csv")


def calc_mse(result):
    predictions = [l[3] for l in result]
    test_val = [l[2] for l in result]
    def squaredDiff(x, y):
        sum = 0.0
        for a,b in zip(x,y):
            sum += 1.0*(a-b)*(a-b)
